...
class TeamBuilderFrame(tk.Frame):
    ...
    def _build_card(self, idx):
        ...
        name_lbl = tk.Label(
            card, text="Select Pokémon", font=TEAM_NAME_FONT, fg=MUTED_COLOR, bg=CARD_BG
        )
    ...
    def _open_dd(self, idx):
        ...
        dd = tk.Frame(
            self.ctrl,
            bg=CARD_BG,
            width=TEAM_DROPDOWN_WIDTH,
            height=TEAM_DROPDOWN_HEIGHT,
        ...
        srch_ent = tk.Entry(
            dd,
            textvariable=srch_var,
            font=TEAM_SEARCH_FONT,
            bg=INPUT_BG_COLOR,
            fg=TXT,
    ...
    def _select_poke(self, idx, poke_data):
        try:
            ...
            self._close_dd()
            self.wgts.append(
                self.err.show(str(e), lambda: self._select_poke(idx, poke_data))
            )
        except PokeAPIError as e:
            self._close_dd()
            self.wgts.append(self.err.show(str(e)))
            ...
            for i, (type_name, cnt) in enumerate(sorted_weak[:8]):
                ...
                if icon:
                    ...
                    clr = (
                        ACCENT_COLOR
                        if cnt >= 3
                        else (GOLD_COLOR if cnt >= 2 else MUTED_COLOR)
                    )
                    badge = tk.Label(
                        cell,
                        text=f"×{cnt}",
                        font=TEAM_BADGE_FONT,
                        fg=clr,
                        bg=ANALYTICS_SECTION_BG,
                    )
                    badge.pack()
    ...
    def _build_stats(self, parent, team):
        ...
        for key in stats:
            ...
            bar_bg = tk.Frame(
                row, bg=INPUT_BG_COLOR, width=STAT_BAR_WIDTH, height=STAT_BAR_HEIGHT
            )
    ...
    def _show_empty_warn(self):
        ...
        popup = tk.Frame(
            self.ctrl,
            bg=CARD_BG,
            width=EMPTY_WARN_WIDTH,
            height=EMPTY_WARN_HEIGHT,
        ...
        tk.Label(
            popup, text="Your team is empty", font=BODY_FONT, fg=MUTED_COLOR, bg=CARD_BG
        ).place(relx=0.5, rely=0.55, anchor="center")

        ok_btn = tk.Label(
            popup,
            text="Got it",
            font=TEAM_OK_BTN_FONT,
            fg=HIGHLIGHT_COLOR,
            bg=ACCENT_COLOR,
            cursor="hand2",
            padx=20,
