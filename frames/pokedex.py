...
class PokedexFrame(tk.Frame):
    ...
    def _click_lbl(self, parent, img, x, y, cb):
        if not img:
            return None
        lbl = tk.Label(
            parent, image=img, bg=BG, cursor="hand2", bd=0, highlightthickness=0
        )
        lbl.image = img
        lbl.place(x=x, y=y)
    ...
    def _get_anim(self, pid, size, shiny=False):
        ...
        try:
            ...
            try:
                while True:
                    frames.append(
                        ImageTk.PhotoImage(gif.copy().resize(size, Image.NEAREST))
                    )
    ...
    def _build_search(self):
        ...
        tk.Label(fr, text="Search:", font=BODY_FONT, fg=TXT, bg=BG).pack(
    ...
    def _build_filter(self):
        ...
        self.filter_fr.place(
            x=FILTER_X, y=FILTER_Y, width=FILTER_WIDTH, height=FILTER_HEIGHT
        )
        ...
        for i, t in enumerate(POKEMON_TYPES):
            ...
            cb = tk.Checkbutton(
                row1 if i < TYPES_PER_ROW else row2,
                text=t,
                variable=var,
                font=FILTER_FONT,
                fg=TXT,
                bg=BG,
                selectcolor=INPUT_BG_COLOR,
                activebackground=BG,
    ...
    def _card(self, poke, idx):
        ...
        card.bind(
            "<Enter>", lambda e: card.configure(highlightbackground=HIGHLIGHT_COLOR)
        )
        ...
        if photo:
            spr = tk.Label(
                ...
                image=photo,
                bg=CARD_BG,
                width=CARD_SPRITE_SIZE[0],
                height=CARD_SPRITE_SIZE[1],
            )
            spr.image = photo
            spr.pack(pady=8)
            ...
            if poke["id"] <= MAX_ANIMATED_ID:
                spr.bind(
                    "<Enter>",
                    lambda e, p=poke, l=spr, img=photo: self._card_enter(
                        p, l, img, card
                    ),
                )
        ...
        lbl = tk.Label(
            row, text=f"#{poke['id']}", font=POKE_ID_FONT, fg=MUTED_COLOR, bg=CARD_BG
        )
    ...
    def _nav_btns(self):
        ...
        lbl = tk.Label(
            self.ctrl,
            text=f"Page {self.page + 1} of {total}",
            font=NAV_FONT,
            fg=TXT,
            bg=BG,
        )
    ...
    def _left_card(self, poke):
        ...
        def enter(e):
            self.shiny_btn.configure(
                fg=GOLD_COLOR if not self.shiny else MUTED_COLOR, bg=HOVER_BG_COLOR
            )

        def leave(e):
            self.shiny_btn.configure(
                fg=GOLD_COLOR if self.shiny else MUTED_COLOR, bg=INPUT_BG_COLOR
            )
        ...
        row.pack(pady=(10, 0))
        tk.Label(
            row, text=poke["name"], font=POKE_DETAIL_NAME_FONT, fg=TXT, bg=CARD_BG
        ).pack(side=tk.LEFT, padx=(0, 15))
        tk.Label(
            row,
            text=f"#{poke['id']}",
            font=POKE_DETAIL_ID_FONT,
            fg=MUTED_COLOR,
            bg=CARD_BG,
        ).pack(side=tk.LEFT)
    ...
    def _weak_section(self, parent, poke):
        ...
        tk.Label(fr, text="Weak to:", font=SECTION_FONT, fg=TXT, bg=CARD_BG).pack(
            anchor="w"
        )
    ...
    def _evo_section(self, parent, poke):
        ...
        tk.Label(cont, text="Evolution:", font=SECTION_FONT, fg=TXT, bg=CARD_BG).pack(
            anchor="w"
        )
    ...
    def _evo_sprite(self, parent, evo, cur_id, idx):
        ...
        if idx > 0:
            tk.Label(fr, text="→", font=ARROW_FONT, fg=MUTED_COLOR, bg=CARD_BG).pack(
                side=tk.LEFT, padx=(0, 5)
            )

        cont = tk.Frame(
            fr, bg=CARD_BG, highlightbackground=CARD_BG, highlightthickness=2
        )
        ...
        if photo:
            ...
            name = tk.Label(
                ...
                text=evo["name"],
                font=EVO_NAME_FONT,
                fg=GOLD_COLOR if is_cur else TXT,
                bg=CARD_BG,
                cursor="arrow" if is_cur else "hand2",
            )
            name.pack(pady=(0, 3))
    ...
    def _close_detail(self):
        ...
        self.wgts[1].place(
            x=SEARCH_X, y=SEARCH_Y, width=SEARCH_WIDTH, height=SEARCH_HEIGHT
        )
        self.filter_fr.place(
            x=FILTER_X, y=FILTER_Y, width=FILTER_WIDTH, height=FILTER_HEIGHT
        )
